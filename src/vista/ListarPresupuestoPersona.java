...
public class ListarPresupuestoPersona extends javax.swing.JPanel  {
    private LocalDate fechaInicio;
    private LocalDate fechaFin;
    
    public ListarPresupuestoPersona(LocalDate fechaInicio,LocalDate fechaFin) {
    ...
    private void listarCuenta(){
        ...
        ListarPresupuestoPersona.tabla1= new JTable(model);
        ListarPresupuestoPersona.jScrollPane2.setViewportView(ListarPresupuestoPersona.tabla1);
    ...
    private void listarIngresos(){
        ...
        DefaultTableModel model=new DefaultTableModel();
        ListarPresupuestoPersona.tabla2= new JTable(model);
        ListarPresupuestoPersona.jScrollPane3.setViewportView(ListarPresupuestoPersona.tabla2);
        model.addColumn("Código");
    ...
    private List<Ingreso> cargaIngresos(){
        ...
        Connection conn=Conexion.conectar();
        String sql1="SELECT * FROM ingreso  WHERE (fecha_inicio>=? OR fecha_fin<=?)AND codigo_persona=?;";
        PreparedStatement pst;
        try{
    ...
    private void listarGastos(){
        ...
        DefaultTableModel model=new DefaultTableModel();
        ListarPresupuestoPersona.tabla= new JTable(model);
        ListarPresupuestoPersona.jScrollPane1.setViewportView(ListarPresupuestoPersona.tabla);
        model.addColumn("Código");
        model.addColumn("Tipo");
        model.addColumn("Descripción");
        model.addColumn("Cantidad");
    ...
    private List<Gasto> cargaGastos(){
        ...
        Connection conn=Conexion.conectar();
        String sql="select * from gasto g where codigo_persona= ? AND (fecha_inicio>=? AND fecha_fin<=?);";
        PreparedStatement pst;
        try{
            pst=conn.prepareStatement(sql);
            pst.setString(1, FrmMenu.pers.getDni());
            pst.setDate(2, Date.valueOf(fechaInicio));
            pst.setDate(3, Date.valueOf(fechaFin));
            ResultSet rs=pst.executeQuery();
            while(rs.next()){
